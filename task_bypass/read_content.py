    ...
    if 'task_inputs' in inputs:
        ...
        if function == 'http-request':
            ...
            for single_df in _from_output:
                ...
                result_df = asyncio.run(http_request(db, stage_name, task_id, single_df, extract_field, preserve_origin_data, concurrent))
        ...
        if function == 'http-request':
            if file_format == 'csv':
                ...
                for row in rows:
                    ...
                    result_df = asyncio.run(http_request(db, stage_name, task_id, row_df, concurrent=concurrent))
                    # add dataframe into lists (produce list of dataframes)
                    result_lists.append(result_df)
            ...
            if base_url:
                ...
                result_df = asyncio.run(http_request(db, stage_name, task_id, params_df, concurrent=concurrent))
